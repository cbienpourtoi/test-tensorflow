...
# biases b[10]
b1 = tf.Variable(tf.ones([K])/10.) # Init biais un peu arbitraire... Prendre petite valeur positive quand on utilise des relu
...
# The model
Y1 = tf.nn.relu(tf.nn.conv2d(X, W1, strides=[1, 1, 1, 1], padding='SAME') + b1) # On le passe avec un pas de 1 pixel
#Y1b = tf.nn.dropout(Y1, pkeep) # No dropouts parce que ici on ne doit pas perdre d infos entre ceux deux couches (l'info n est pas sparse) 

# The model
Y2 = tf.nn.relu(tf.nn.conv2d(Y1, W2, strides=[1, 2, 2, 1], padding='SAME') + b2) # Pas de 2 pix
...
# The model
Y4 = tf.nn.relu(tf.matmul(tf.reshape(Y3, shape = [-1, 7 * 7 * M]), W4) + b4)
...
datavis.animate(training_step, iterations=15000+1, train_data_update_freq=40, test_data_update_freq=50, more_tests_at_start=True)
